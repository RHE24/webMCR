...
function LoadServersProc() {
    ...
    var event = function(response) {
        ...
        if (response['online'] == 1) {
            ...
            if (response['numpl'] > response['slots'])
                response['numpl'] = response['slots']
            if (response['slots'] != -1)
            ...
            BlockVisible('off-sbyid-' + item_id, true)
        ...
        if (response['pl_array'].length != 0 && response['numpl'] > 0) {
            ...
            if (players_block != null) {
                ...
                for (i = 1; i <= players_arr.length - 1; ++i)
                    formatStr += ', <b>' + players_arr[i] + '</b>'
...
function ProgressBarManager(className, static_text) {
    ...
    if (className != null) {
        ...
        for (i = 0; i <= tmpBars.length - 1; ++i)
            PBars[i] = {bar: tmpBars[i], main: GetParent(tmpBars[i], 'DIV'), static_text: static_text, dead: false}
        delete tmpBars
    }
    ...
    function getDivsByClass(className) {
        ...
        for (i = 0; i <= divList.length - 1; ++i)
            if (divList[i].className == className)
                foundList[foundList.length] = divList[i]
    ...
    function AnimateBar(pid, from, to) {
        ...
        if (from == null)
            from = parseIntZero(PBars[pid].bar.style.width)
        if (from < PBarMinWidth)
            from = PBarMinWidth
        if (to == null || to < PBarMinWidth)
            to = PBarMinWidth
        ...
        if (from < to)
            ...
            PBars[pid].bar.style.width = to + '%'
    }

    function MoveImage(pid) {
        if (PBars[pid].dead || PBars[pid].main == null || PBars[pid].main.style.display == 'none')
            return
