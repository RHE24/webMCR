...
getDB()->ask("CREATE TABLE IF NOT EXISTS `{$bd_names['users']}` (
  ...
  `{$bd_users['password']}` " . EncoderPreset::getColName() . " DEFAULT NULL,
  ...
  `default_skin` tinyint(1) NOT NULL DEFAULT '2',
  `{$bd_users['session']}` varchar(32) DEFAULT NULL,
  `{$bd_users['clientToken']}` varchar(32) DEFAULT NULL,
  `{$bd_users['server']}` varchar(32) DEFAULT NULL,  
