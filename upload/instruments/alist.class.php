...
class ThemeManager extends View
{
    ...
    public function __construct($style_sd = false, $work_skript = '?mode=control')
    {
    ...
    public static function DeleteTheme($theme_id)
    {
        ...
        if ($theme_id === View::DEFAULT_THEME)
            return false;
        ...
        if ($config['s_theme'] === $theme_id) {
            ...
            $config['s_theme'] = View::DEFAULT_THEME;
    ...
    public static function DownloadTInstaller($theme_id)
    {
        ...
        $tmp_base_dir = getWay('tmp');
        $tmp_file = tempnam($tmp_base_dir, 'zip');

        if ($tmp_file === false) return false;
        
        // some windows servers fail to create zip by ZipArchive module, do that manualy
    ...
    public function ShowThemeSelector()
    {
        ...
        $theme_cur = isset($config['s_theme']) ? $config['s_theme'] : View::DEFAULT_THEME;
        ...
        foreach ($theme_list as $key => $theme_info)
            include $this->getView('theme_item.html');

        $theme_items_info = ob_get_clean();

        ob_start();

        include $this->getView('theme_select.html');
        ...
        for ($i = 0; $i < sizeof($theme_info_txt); $i++) {
            ...
            for ($b = 0; $b < sizeof(self::$true_info); $b++) {
                ...
                if (sizeof($info_value) == 2)
                    $theme_info[self::$true_info[$b]] = Filter::str(preg_replace('/\s{2,}/', ' ', nl2br($info_value[1])));
            }
        }
...
class ControlManager extends View 
{
    ...
    public function ShowUserListing($list = 1, $input) 
    {
    global $bd_users,$bd_names;
        ...
        $result = getDB()->ask($sql.$whereSql.$orderSql, $input); 
        ...
        while ( $line = $result->fetch('num') ) {
            ...
            $user_id   = $inf_user->id();
            $user_ip   = $inf_user->ip();
            $user_group  = $inf_user->getGroupName();
        ...
        $line = getDB()->fetchRow($countSql.$whereSql.$orderSql, $input, 'num');
        $html .= $this->showArrows($this->work_skript, $list, $line[0], 10);
    ...
    global $bd_names;
        ...
        include $this->GetView('server/servers_caption.html');
        ...
        while ( $line = $result->fetch() ) {
            ...
            if (!isset($found)) {
                ...
                $found = true;
                include $this->GetView('server/servers_header.html'); 
            }
            ...
            include $this->GetView('server/servers_string.html');         
        }
        
 	if (!isset($found)) { include $this->GetView('server/servers_not_found.html'); return ob_get_clean(); }  
		       
	include $this->GetView('server/servers_footer.html'); 
	$html = ob_get_clean();
