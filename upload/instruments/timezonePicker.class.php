...
class TimezonePicker
{
    ...
    public static function showOptions($selected = false)
    {
        $html = '';
        
        if (!$selected) $selected = date_default_timezone_get();
    ...
    public static function show($name = '', $class = '', $selected = false) 
    {
        return '<select name="'.$name.'" class="'.$class.'">' . self::showOptions($selected) . '</select>';
    }

    public static function isExist($timezone)
    {
        return (bool) array_search($timezone, self::$timezones);
    }
}
